...
struct jit_uni_reorder_kernel_f32: public kernel_t, public jit_generator_aarch64 {
    ...
    enum {
        /* From the innter most dimension, unrolled until # of data reaches this value. */
        len_unroll_max = 256, 
        /* From the innter most dimension, calculate how many dimensions can be unrolled.
           If the unrollable dimension is less than this value, JIT code generation is canceled. */
        ndims_jit_loop_max = 3, 
    };

    struct simple_impl_desc_t {
        int ndims_full_unroll; // # of full unrolled dimension
    ...
    static bool simple_impl_desc_init(const prb_t &prb,
        ...
        int len_unroll = 1; // # of data unrolled
...
// 7の範囲
    ...
    void process_unroll_generic_step(int reg_unroll, const int *i_off,
        ...
        /* srcがアドレスの場合 */
        auto cvt2ps_addr = [=](const VReg &dst, const Xbyak::Xbyak_aarch64::AdrImm &src,
                                   data_type_t idt) {
				
            mov(reg_tmp, XReg(src.getXn()));
            add(reg_tmp, reg_tmp, src.getImm());
            Xbyak::Xbyak_aarch64::AdrNoOfs addr(reg_tmp);
	  ...
	  for (int ur = 0; ur < reg_unroll; ur += load_step) {
        ...
        if (can_store_xmm) {
			...
			rsvdOffsetScale = s_off[ur] * stype_sz;
		      ...
		      Xbyak::Xbyak_aarch64::AdrPostImm addr(reg_tmpScale, 4);
			...
			rsvdOffsetOut = tmpOffset;
		      ...
		      Xbyak::Xbyak_aarch64::AdrPostImm addr(reg_tmpOut, 4);
                        ld1(xmm_tmp.s4, addr);
			rsvdOffsetOut += 4;
    ...
    void loop_begin(LabelAArch64 &l, Reg64 reg_cnt, int len) {
        mov(reg_cnt, len);
        L(l);
    }

    void loop_end(LabelAArch64&l, Reg64 reg_cnt, int len, int i_step, int o_step,
            int s_step) {
	...
	eor(reg_off_out, reg_off_out, reg_off_out);
	  ...
	  eor(reg_off_scale, reg_off_scale, reg_off_scale);
        ...
        const bool optimized = false
            || process_direct_copy_sve(d.len_unroll)
