    ...
    rnn_postgemm_dispatcher(
        ...
        case alg_kind::vanilla_lstm:
            ...
            if (pd->desc()->prop_kind == prop_kind::forward_inference) {
				...
				if (mayiuse(avx512_core))
                    rnn_postgemm_
                            = new jit_uni_lstm_cell_postgemm_fwd<avx512_core,
                                    src_type>(rnn, pd);
                else if (mayiuse(avx2))
					rnn_postgemm_ = new jit_uni_lstm_cell_postgemm_fwd<avx2,
                            src_type>(rnn, pd);
                else if (mayiuse(sse42))
					rnn_postgemm_ = new jit_uni_lstm_cell_postgemm_fwd<sse42,
                            src_type>(rnn, pd);
        ...
        case alg_kind::vanilla_rnn:
            ...
            if (pd->desc()->prop_kind == prop_kind::forward_inference) {
				...
				if (mayiuse(avx512_core))
                    ...
                    rnn_postgemm_ = new jit_uni_rnn_cell_postgemm_fwd<sse42,
                            src_type>(rnn, pd);
#endif // #ifdef __ARM_ARCH
			}
            if (rnn_postgemm_)
                ...
                case alg_kind::eltwise_logistic:
                    activation_func
                            = &activation<alg_kind::eltwise_logistic, aprop>;
                    break;
                default: break;
                }
            }
        ...
        case alg_kind::vanilla_gru:
            ...
            if (pd->desc()->prop_kind == prop_kind::forward_inference) {
				...
				if (mayiuse(avx512_core)) {
                    ...
                    rnn_postgemm_
                            = new jit_uni_gru_cell_postgemm_part1_fwd<sse42,
                                    src_type>(rnn, pd);
                    rnn_postgemm_part2_
        ...
        case alg_kind::gru_linear_before_reset:
            ...
            if (pd->desc()->prop_kind == prop_kind::forward_inference) {
				...
				if (mayiuse(avx512_core))
                    rnn_postgemm_
                            = new jit_uni_gru_lbr_cell_postgemm_fwd<avx512_core,
                                    src_type>(rnn, pd);
                else if (mayiuse(avx2))
                    rnn_postgemm_ = new jit_uni_gru_lbr_cell_postgemm_fwd<avx2,
                            src_type>(rnn, pd);
                else if (mayiuse(sse42))
                    rnn_postgemm_ = new jit_uni_gru_lbr_cell_postgemm_fwd<sse42,
                            src_type>(rnn, pd);
#endif // 			#ifdef __ARM_ARCH
