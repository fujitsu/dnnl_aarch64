...
#ifdef __ARM_ARCH
	typedef typename utils::conditional<isa == sve,
		jit_uni_eltwise_injector_f32<sve>,
...
protected:
	...
	const primitive_attr_t *attr = pd_->attr();
        ...
        // Register map
        XReg loop_cnt(x11);  // loop counter
		XReg table_reg(x1); // table is used for data scale and shifts
		XReg tmp_reg(x12);   // used as temporary to customize mxcsr
		XReg weights_scales_reg(x13);
...
#ifdef __ARM_ARCH
template struct jit_uni_rnn_cell_postgemm_fwd<sve, data_type::f32>;
template struct jit_uni_rnn_cell_postgemm_fwd<sve, data_type::u8>;
#else
