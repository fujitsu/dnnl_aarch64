...
struct jit_uni_reorder_kernel_f32: public kernel_t, public jit_generator {
    ...
    void process_unroll_generic_step(int reg_unroll, const int *i_off,
        ...
        auto cvt2ps = [=](const VReg &dst, const Operand &src, data_type_t idt) {
            VReg dst_pure = VReg(dst.getIdx());
            switch (idt) {
            case f32:
        ...
        auto cvt2int = [=](const Xmm &xmm, data_type_t odt, data_type_t idt) {
            ...
            case s32:
                ...
                break;
            case s8:
	      if (idt == f32) vcvtps2dq(xmm, xmm); // Convert to signed 32-bit integer x 4 elements with saturation.
                if (idt == f32 || idt == s32) {
                    if (mayiuse(avx512_core)) {
                        vpmovsdb(xmm, xmm);
                    } else {
		      vpackssdw(xmm, xmm, xmm_zero); // Convert to signed 16-bit integer x 4 elements with saturation.
		      vpacksswb(xmm, xmm, xmm_zero); // Convert to signed 8-bit integer x 4 elements with saturation.
                    }
                }
                if (idt == u8) vpminub(xmm, xmm, xmm_4x127b); // 比較して小さい方を選択する
                break;
            case u8:
                if (idt == f32) vcvtps2dq(xmm, xmm);
                if (idt == f32 || idt == s32) {
        ...
        auto load = [=](const Xmm &xmm, const Address &addr, int size) {
            switch (size) {
            case 16: movups(xmm, addr); break; // load 128 bits. No alignment is needed.
            case 4: movss(xmm, addr); break; // load 32 bits
            case 1: pinsrb(xmm, addr, 0x0); break; // load 8 bits (LSB). Other bits are 0 cleared.
            default: assert(!"unreachable");
            }
        };
    ...
    bool simple_impl() {
        ...
        eor(reg_off_in, reg_off_in);
        eor(reg_off_out, reg_off_out);
        if (prb_.scale_type == scale_type_t::MANY)
    ...
    jit_uni_reorder_kernel_f32(const desc_t &desc)
        ...
        preamble(); // ABI仕様に基づき、待避が必要なレジスタの待避
...
#       undef PARAM
        ...
        if (mayiuse(avx)) {
            vxorps(xmm_zero, xmm_zero, xmm_zero); // xmm14を0クリア

            if (prb_.itype == data_type::u8 && prb_.otype == data_type::s8) { // maskデータ生成
