...
template <cpu_isa_t> struct cpu_isa_traits {}; 

template <>
struct cpu_isa_traits<neon64> {
...
template <> struct cpu_isa_traits<sve> {
    typedef Xbyak::ZRegS Vmm;
	typedef Xbyak::AdrScImm uni_ldst_addressing;
	static constexpr int vlen_shift = 5;
...
namespace {
	...
	static inline bool mayiuse(const cpu_isa_t cpu_isa) {
		...
		switch (cpu_isa) {
		case neon64:
			return cpu.has(Cpu::tNEON64);
        case neon128: 
			return cpu.has(Cpu::tNEON128);
        case simd: 
			return cpu.has(Cpu::tSIMD);
		case sve:
			return cpu.has(Cpu::tSVE);
