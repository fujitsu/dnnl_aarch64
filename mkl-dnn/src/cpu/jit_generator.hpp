...
static const Xbyak::XReg abi_param1(Xbyak::Operand::X0),
...
public:
  ...
  class XRegValue : public Xbyak::XReg {
  public:
    int64_t value_;
    ...
    void preamble() {
	  ...
	  add(x29, sp, xreg_len * 2);
        ...
        for (size_t i = 0; i < num_abi_save_gpr_regs; i += 2) {
            stp(Xbyak::XReg(abi_save_gpr_regs[i]),
                    Xbyak::XReg(abi_save_gpr_regs[i + 1]), post_ptr(x29, xreg_len*2));
	}
    }
#else
    Xbyak::Reg64 param1 = abi_param1;
  ...
  template<class T> void uni_reg_clear(const T& r) {
    ...
    void postamble() {
	  ...
	  ld4((v12.d - v15.d)[0], post_ptr(x29, vreg_len_preserve*4));
        ...
        for (size_t i = 0; i < num_abi_save_gpr_regs; i += 2) {
            ldp(Xbyak::XReg(abi_save_gpr_regs[i]),
                    Xbyak::XReg(abi_save_gpr_regs[i + 1]), post_ptr(x29, xreg_len*2));
	}
    ...
    /*
      template <typename T>
      Xbyak::Adr EVEX_compress_addr(Xbyak::XReg base,
    ...
    */
    ...
    Xbyak::Adr make_safe_addr(const Xbyak::XReg &reg_out, size_t offt,
            const Xbyak::XReg &tmp_reg, bool bcast = false) {

        assert(NULL);
        return ptr(x0);
    }

    Xbyak::Adr EVEX_compress_addr_safe(const Xbyak::XReg &base, size_t raw_offt,
            const Xbyak::XReg &reg_offt, bool bcast = false) {
    ...
    // WREG, XREG, VREG, ZREG
#if 0  
  void uni_vpxor(const Xbyak::VRreg &x1, const Xbyak::VReg &x2, const Xbyak::VRreg &op) {
    assert(x1.getIdx() == x2.getIdx());
    xor(x2, x2, op);
  }

  void uni_vpxor(const Xbyak::ZReg &x1, const Xbyak::ZReg &x2, const Xbyak::ZReg &op) {
    assert(x1.getIdx() == x2.getIdx());
    xor(x2, x2, op);
  }

  void uni_vmovss(const Xbyak::Adr& addr, const Xbyak::QReg& x) {
    str(x, addr);
  }

  void uni_vmovss(const Xbyak::Adr& addr, const Xbyak::ZReg& x) {
    str(x, addr);
  }
  ...
  void uni_vmovss(const Xbyak::ZReg &x, const Xbyak::Adr& addr) {
    ldr(x, addr);
  }
#endif // #if 0
    void uni_vpxor(const Xbyak::QReg &x1, const Xbyak::QReg &x2,
            const Xbyak::QReg &op) {
        assert(NULL);
    }
    void uni_vpxor(const Xbyak::ZReg &x1, const Xbyak::ZReg &x2,
            const Xbyak::ZReg &op) {
        assert(NULL);
    }
    ...
    void uni_vmovss(const Xbyak::QReg &x, const Xbyak::Adr &addr) {
        assert(NULL);
    }
    void uni_vmovss(const Xbyak::ZReg &x, const Xbyak::Adr &addr) {
        assert(NULL);
    ...
    void uni_vmovsd(const Xbyak::Adr &addr, const Xbyak::QReg &x) {
        assert(NULL);
    }
    void uni_vmovsd(const Xbyak::Adr &addr, const Xbyak::ZReg &x) {
        assert(NULL);
    }
    void uni_vmovsd(const Xbyak::QReg &x, const Xbyak::Adr &addr) {
        assert(NULL);
    }
    void uni_vmovsd(const Xbyak::ZReg &x, const Xbyak::Adr &addr) {
        assert(NULL);
    }

    void uni_vmovdqu(const Xbyak::Adr &addr, const Xbyak::QReg &x) {
        assert(NULL);
    }
    void uni_vmovdqu(const Xbyak::Adr &addr, const Xbyak::ZReg &x) {
        assert(NULL);
    }

    void uni_vmovdqu(const Xbyak::QReg &x, const Xbyak::Adr &addr) {
        assert(NULL);
    }
    void uni_vmovdqu(const Xbyak::ZReg &x, const Xbyak::Adr &addr) {
        assert(NULL);
    }

    void uni_vmovups(const Xbyak::Adr &addr, const Xbyak::QReg &x) {
        assert(NULL);
    }
    void uni_vmovups(const Xbyak::Adr &addr, const Xbyak::ZReg &x) {
        assert(NULL);
    }

    void uni_vmovups(const Xbyak::QReg &x, const Xbyak::Operand &op) {
        assert(NULL);
    }
    void uni_vmovups(const Xbyak::ZReg &x, const Xbyak::Operand &op) {
        assert(NULL);
    }

    void uni_vmovntps(const Xbyak::Adr &addr, const Xbyak::QReg &x) {
        assert(NULL);
    }
    void uni_vmovntps(const Xbyak::Adr &addr, const Xbyak::ZReg &x) {
        assert(NULL);
    }

    void uni_vbroadcastss(const Xbyak::QReg &x, const Xbyak::Operand &op) {
        assert(NULL);
    }
    void uni_vbroadcastss(const Xbyak::ZReg &x, const Xbyak::Operand &op) {
        assert(NULL);
    }

    void uni_vpbroadcastd(const Xbyak::QReg &x, const Xbyak::Operand &op) {
        assert(NULL);
    }
    void uni_vpbroadcastd(const Xbyak::ZReg &x, const Xbyak::Operand &op) {
        assert(NULL);
    }

    void uni_vrcpss(const Xbyak::QReg &x, const Xbyak::Operand &op) {
        assert(NULL);
    }
    void uni_vrcpss(const Xbyak::ZReg &x1, const Xbyak::QReg &x2) {
        assert(NULL);
    }
    void uni_vrcpss(const Xbyak::ZReg &x, const Xbyak::Adr &op) {
        assert(NULL);
    }

    void uni_vrcpps(const Xbyak::QReg &x, const Xbyak::Operand &op) {
        assert(NULL);
    }
    void uni_vrcpps(const Xbyak::ZReg &x, const Xbyak::Operand &op) {
        assert(NULL);
    }
    ...
    void uni_vdivps(const Xbyak::QReg &x, const Xbyak::Operand &op1,
            const Xbyak::Operand &op2, const Xbyak::QReg &buf) {
        assert(NULL);
    }

    void uni_vdivps(const Xbyak::ZReg &x, const Xbyak::Operand &op1,
            const Xbyak::Operand &op2, const Xbyak::ZReg &buf) {
        assert(NULL);
    }
    ...
    void uni_vpsignd(const Xbyak::QReg &x1, const Xbyak::QReg &x2,
            const Xbyak::Operand &op) {
        assert(NULL);
    }
    void uni_vpsignd(const Xbyak::ZReg &x1, const Xbyak::ZReg &x2,
            const Xbyak::Operand &op) {
        assert(NULL);
    }
    ...
    void uni_vsubps(const Xbyak::QReg &x, const Xbyak::Operand &op1,
            const Xbyak::Operand &op2, const Xbyak::QReg &buf) {
        assert(NULL);
    }

    void uni_vsubps(const Xbyak::ZReg &x, const Xbyak::Operand &op1,
            const Xbyak::Operand &op2, const Xbyak::ZReg &buf) {
        assert(NULL);
    }
    ...
    void uni_vmulss(const Xbyak::ZReg &x, const Xbyak::Operand &op1,
            const Xbyak::Adr &op2) {
        assert(NULL);
    }
    void uni_vmulss(const Xbyak::ZReg &x, const Xbyak::Operand &op1,
            const Xbyak::ZReg &op2) {
        assert(NULL);
    }

    void uni_vfmadd213ps(const Xbyak::QReg &x1, const Xbyak::QReg &x2,
            const Xbyak::Operand &op) {
        assert(NULL);
    }
    void uni_vfmadd213ps(const Xbyak::ZReg &x1, const Xbyak::ZReg &x2,
            const Xbyak::Operand &op) {
        assert(NULL);
    }

    void uni_vfmadd213ss(const Xbyak::QReg &x1, const Xbyak::QReg &x2,
            const Xbyak::Operand &op) {
        assert(NULL);
    }
    void uni_vfmadd213ss(const Xbyak::ZReg &x1, const Xbyak::ZReg &x2,
            const Xbyak::Operand &op) {
        assert(NULL);
    }

    void uni_vfmadd231ps(const Xbyak::QReg &x1, const Xbyak::QReg &x2,
            const Xbyak::Operand &op) {
        assert(NULL);
    }
    void uni_vfmadd231ps(const Xbyak::ZReg &x1, const Xbyak::ZReg &x2,
            const Xbyak::Operand &op) {
        assert(NULL);
    }
    void uni_vfmadd231ss(const Xbyak::QReg &x1, const Xbyak::QReg &x2,
            const Xbyak::Operand &op) {
        assert(NULL);
    }
    void uni_vfmadd231ss(const Xbyak::ZReg &x1, const Xbyak::ZReg &x2,
            const Xbyak::Operand &op) {
        assert(NULL);
    }

    void uni_vfnmadd231ps(const Xbyak::QReg &x1, const Xbyak::QReg &x2,
            const Xbyak::Operand &op) {

        assert(NULL);
    }

    void uni_vfnmadd231ps(const Xbyak::ZReg &x1, const Xbyak::ZReg &x2,
            const Xbyak::Operand &op) {
        assert(NULL);
    }

    void uni_vsqrtps(const Xbyak::QReg &x, const Xbyak::Operand &op) {
        assert(NULL);
    }
    void uni_vsqrtps(const Xbyak::ZReg &x, const Xbyak::Operand &op) {
        assert(NULL);
    }

    void uni_vpaddd(const Xbyak::QReg &x1, const Xbyak::QReg &x2,
            const Xbyak::Operand &op) {
        assert(NULL);
    }
    void uni_vpaddd(const Xbyak::ZReg &x1, const Xbyak::QReg &x2,
            const Xbyak::Operand &op) {
        assert(NULL);
    }
    ...
    void uni_vpslld(
            const Xbyak::QReg &x, const Xbyak::Operand &op, const int imm) {
        assert(NULL);
    }
    void uni_vpslld(
            const Xbyak::ZReg &x, const Xbyak::Operand &op, const int imm) {
        assert(NULL);
    }
    ...
    void uni_vcmpgtps(const Xbyak::QReg &x1, const Xbyak::QReg &x2,
            const Xbyak::Operand &op) {
    ...
    void uni_vcmpgtps(const Xbyak::ZReg &x1, const Xbyak::ZReg &x2,
            const Xbyak::Operand &op) {
    ...
    void uni_vcmpgeps(const Xbyak::QReg &x1, const Xbyak::QReg &x2,
            const Xbyak::Operand &op) {
    ...
    void uni_vcmpgeps(const Xbyak::ZReg &x1, const Xbyak::ZReg &x2,
            const Xbyak::Operand &op) {

        assert(NULL);
    }

    void uni_vtestps(const Xbyak::QReg &x1, const Xbyak::Operand &op) {

        assert(NULL);
    }

    void uni_vtestps(const Xbyak::ZReg &x1, const Xbyak::Operand &op) {

        assert(NULL);
    }

    void uni_vblendvps(const Xbyak::QReg &x1, const Xbyak::QReg &x2,
            const Xbyak::Operand &op, const Xbyak::QReg &msk) {

        assert(NULL);
    }
    void uni_vblendvps(const Xbyak::ZReg &x1, const Xbyak::ZReg &x2,
            const Xbyak::Operand &op, const Xbyak::ZReg &msk) {
    ...
    void uni_vcvtps2dq(const Xbyak::QReg &x, const Xbyak::Operand &op) {

        assert(NULL);
    }
    void uni_vcvtps2dq(const Xbyak::ZReg &x, const Xbyak::Operand &op) {

        assert(NULL);
    }

    void uni_vcvtdq2ps(const Xbyak::QReg &x, const Xbyak::Operand &op) {

        assert(NULL);
    }
    void uni_vcvtdq2ps(const Xbyak::ZReg &x, const Xbyak::Operand &op) {

        assert(NULL);
    }

    void uni_vmovmskps(const Xbyak::Reg &x1, const Xbyak::QReg &x2) {

        assert(NULL);
    }
    void uni_vmovmskps(const Xbyak::Reg &x1, const Xbyak::ZReg &x2) {

        assert(NULL);
    }

    void uni_vpackssdw(const Xbyak::QReg &x1, const Xbyak::QReg &x2,
            const Xbyak::Operand &op) {


        assert(NULL);
    }
    void uni_vpackssdw(const Xbyak::ZReg &x1, const Xbyak::ZReg &x2,
            const Xbyak::Operand &op) {

        assert(NULL);
    }

    void uni_vpackuswb(const Xbyak::QReg &x1, const Xbyak::QReg &x2,
            const Xbyak::Operand &op) {

        assert(NULL);
    }
    void uni_vpackuswb(const Xbyak::ZReg &x1, const Xbyak::ZReg &x2,
            const Xbyak::Operand &op) {

        assert(NULL);
    }

	void uni_ld(const Xbyak::ZRegS &z1s, const Xbyak::AdrScImm &addr) {
		assert(NULL);
	}


	void uni_ld(const Xbyak::VReg4S &v1s4, const Xbyak::AdrScImm &addr) {
		assert(NULL);
	}

	void uni_st(const Xbyak::ZRegS &z1s, const Xbyak::AdrScImm &addr) {
		assert(NULL);
	}

	void uni_st(const Xbyak::VReg4S &v1s4, const Xbyak::AdrScImm &addr) {
		assert(NULL);
	}

	void uni_vcvts2f(const Xbyak::ZRegS &z1s, const Xbyak::ZRegS &z2s,
            const Xbyak::_PReg &p1) {
		assert(NULL);
	}

	void uni_vcvts2f(const Xbyak::VReg4S &v1, const Xbyak::VReg4S &v2,
            const Xbyak::_PReg &p1) {
		assert(NULL);
	}

	void uni_vcvtf2s(
            const Xbyak::ZRegS &z1s, const Xbyak::ZRegS &z2s, const Xbyak
            :: _PReg &p1) {
	}

	void uni_vcvtf2s(const Xbyak::VReg4S &v1, const Xbyak::VReg4S &v2,
            const Xbyak ::_PReg &p1) {}


            void mul_by_const(const Xbyak::XReg &out, const Xbyak::Reg64 &tmp,
                    int value) {
...
public:
    ...
    const Xbyak::XBYAK_CODE_PTR *getCode() {
        const Xbyak::XBYAK_CODE_PTR *code = CodeGenerator::getCode();
        register_code(code);
