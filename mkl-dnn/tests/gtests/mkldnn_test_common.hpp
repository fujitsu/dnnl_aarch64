...
#define ALWAYS_SAME_RANDOM_NUMBER 0
...
static inline data_t set_value(size_t index, data_t mean, data_t deviation,
        double sparsity)
{
#if ALWAYS_SAME_RANDOM_NUMBER
	static uint64_t reg = 0xACE1CE1ACE1FA723;
	uint64_t bit = 0;
	...
	//	reg = (reg >> 4) | (bit << 63);
	//	printf("%04X\n", reg);
#endif //#if ALWAYS_SAME_RANDOM_NUMBER

    if (data_traits<data_t>::data_type == mkldnn::memory::data_type::f32) {
#if ALWAYS_SAME_RANDOM_NUMBER
      const size_t group_size = (size_t)(1. / (reg >> 1));
      const size_t group = (reg >> 2) / group_size;
      const size_t in_group = (reg >> 3) % group_size;
        const bool fill = in_group == ((group % 1637) % group_size);
	//	printf("%s, %d, %016lX64\n", __FILE__, __LINE__, (uint64_t)reg);
	return fill ? reg : data_t{0};
#else
